...
function initUI(_box){
	...
	if ($.fn.xheditor) {
		$("textarea.editor", $p).each(function(){
			...
			op["internalScript"]=true; // 留内部JS代码
			//op["inlineScript"]=false;
			op["internalStyle"]=true; // 保留内部样式
			op["cleanPaste"]=false;
			//op["inlineStyle"]=false; 
			$this.xheditor(op);
		});
	}
	...
	if ($.fn.uploadify) {
		$(":file[uploader]", $p).each(function(){
			...
			var options = {
				...
				method       : "GET", // 只有为GET时才能通过scriptData 传参数
				scriptAccess : "always",
				wmode        : 'transparent',
			...
			if ($this.attr("sizeLimit")) {
                options.sizeLimit = $this.attr("sizeLimit");
            }
			if ($this.attr("height")) {
			...
			if ($this.attr("buttonImg")) {
                options.buttonImg = $this.attr("buttonImg");
            }
			if ($this.attr("auto")) {
			    options.auto = $this.attr("auto")=="true";
			}
			if ($this.attr("multi")) {
                options.multi = $this.attr("multi")=="true";
            }
