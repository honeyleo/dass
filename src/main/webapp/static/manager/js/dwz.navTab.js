...
var navTab = {
	...
	_switchTab: function(iTabIndex){
		...
		//切换dwz页面标签时左边树状菜单栏同步高亮
		var tableId =  $tab.attr("tabid");
		changeSelNode(tableId);
...
function changeSelNode(navTabId){
	var unselectedTagAList=$("a[class='node']");
	for(var i=0; i < unselectedTagAList.length; i++){
		var unselectedTagA  = unselectedTagAList[i];
		id = unselectedTagA.id;
		if(id != null && id != ""){
			var unselectedTagAJQ = $("#"+id);
			if(unselectedTagAJQ.attr("rel") == navTabId){
				var idNum = id.split("sleftMenu")[1];
				leftMenu.s(idNum);
			}
		}
	}
