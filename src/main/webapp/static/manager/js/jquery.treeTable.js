	...
	$.fn.treeTable = function (opts) {
        ...
        function formatNode(tr) {
            ...
            if (cMap[id] == 0) {
                ...
                var $preSpan = $('<span class="prev_part"></span>');
            } else {
                //先判断是否有上一个兄弟节点
                if (!$cur.attr('isFirstOne')) {
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTable).children("td").eq(opts.column).find('.prev_part').clone();
                } else {
                    var $parent = $('#' + cMap[id], $treeTable);
                    //没有上一个兄弟节点，则使用父节点的prev_span
                    var $preSpan = $parent.children("td").eq(opts.column).find('.prev_part').clone();
